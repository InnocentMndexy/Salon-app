      ...
      <div class="row" style="margin-top: 50px;">
        <div class="col-sm-6">
          <div class="card" style="background-color: rgba(255, 255, 255, 0.685);">
            <div class="card-body">
              <form action="./complete-oder.php" method="post">
                <div class="mb-3">
                  <input type="text" class="form-control" id="exampleFormControlInput1" readonly value="<?php echo $_SESSION['style']; ?>" required>
                </div>
                <div class="mb-3">
                  <select class="form-control" name="serviceLocation" aria-label="Default select example" required>
                    <option selected>Service Location</option>
                    <option value="Salon">Salon</option>
                    <option value="House"><a href="index.php">House call</a></option>
                  </select>
                </div>
                <div class="mb-3">
                  <select class="form-control" name="empId" aria-label="Default select example" required>
                    <option selected>Available Hair Stylist</option>
                    <?php while($row = $resultSelectAvailableStylist->fetch_assoc()): ?>
                      <option value="<?php echo $row['empID']; ?>"><?php echo $row['empName'].' '.$row['empSurname']; ?></option>
                    <?php endwhile; ?>
                  </select>
                </div>
                <div class="mb-3">
                  <input type="date" name="date" min="" class="form-control" id="datefield" required>
                </div>
                <div class="mb-3">
                  <input type="time" name="time" min="08:00" max="17:00" class="form-control" required>
                  <small>Office hours are 8am to 4pm</small>

                </div>
                <button type="submit" name="proceedToPayMent" class="btn btn-success">Proceed to payment</button>
              </form>
            </div>
            <hr>
          </div>
